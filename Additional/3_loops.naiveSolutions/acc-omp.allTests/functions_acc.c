                    ...
                    double T_new[gridx+2][gridy+2])
{
   ...
   //   #pragma acc loop independent //to activate together with kernels above
        //:pgi:justacc:static(non-managedMemory):works (slow:this naive-version has data transfers in every call)
        //:pgi:justacc:static-managedMemory:works (slow:this naive-version has data transfers in every call)
        //:pgi:justacc:dynamic(non-managedMemory):works (slow:this naive-version has data transfers in every call)
        //:pgi:justacc:dynamic-managedMemory:works (slow:this naive-version has data transfers in every call)
   ...
   #pragma acc parallel loop copyin(T[:gridx+2][:gridy+2]) copyout(T_new[:gridx+2][:gridy+2]) collapse(2)
        ...
        //:pgi:justacc:static-managedMemory:works (slow:this naive-version has data transfers in every call)
        //:pgi:justacc:dynamic(non-managedMemory):works (slow:this naive-version has data transfers in every call)
        //:pgi:justacc:dynamic-managed:works (slow:this naive-version has data transfers in every call)
   for(i = 1; i <= gridx; i++) 
      //#pragma acc loop independent //to activate together with kernels above
      for(j = 1; j <= gridy; j++) 
